<template>
    <ul class="navList">
        ...
        <li v-for="(item, index) in menuArray">
            ...
            <template v-else>
                <div @click="menuopen(index)" class="navList__subheading row row--align-v-center" >
                    <span class="navList__subheading-icon"><i :class="item.icon"></i></span>
                    <span class="navList__subheading-title">{{ item.isOpen }}</span>
                </div>
                <ul :class="item.isOpen">
                    <li v-for="(itemm) in item.childs"
                        @click="open(itemm.route,itemm.name)"
                        class="subList__item">
                        <span class="far fa-dot-circle"></span>{{' '+itemm.name }}
                    </li>
                </ul>
            </template>
...
<script>
    ...
    export default {
        ...
        methods:{
            ...
            menuopen(payload) {
                console.log(payload)
               this.$store.dispatch('general/openCloseMenu',payload)
            },
        },
        computed:mapState({
